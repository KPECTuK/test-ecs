...
namespace Leopotam.EcsLite
{
	...
	public class EcsSystems : IEcsSystems
	{
		...
		public virtual IEcsSystems AddWorld(EcsWorld world, string name)
		{
			...
			if(_inited)
			{
				throw new Exception("Cant add world after initialization.");
			}
			if(world == null)
			{
				throw new Exception("World cant be null.");
			}
			if(string.IsNullOrEmpty(name))
			{
				throw new Exception("World name cant be null or empty.");
			}
			if(_worlds.ContainsKey(name))
			{
				throw new Exception($"World with name \"{name}\" already added.");
			}
			#endif
			_worlds[name] = world;
			return this;
		...
		public virtual void Init()
		{
			...
			foreach(var system in _allSystems)
			{
				if(system is IEcsPreInitSystem initSystem)
				{
					...
					if(worldName != null)
					{
						throw new Exception($"Empty entity detected in world \"{worldName}\" after {initSystem.GetType().Name}.PreInit().");
					}
					#endif
				}
			}
			foreach(var system in _allSystems)
			{
				if(system is IEcsInitSystem initSystem)
				{
					...
					if(worldName != null)
					{
						throw new Exception($"Empty entity detected in world \"{worldName}\" after {initSystem.GetType().Name}.Init().");
					}
					#endif
				}
			}
		...
		public virtual void Run()
		{
			...
			#endif
			for(int i = 0,
				iMax = _runSystems.Count; i < iMax; i++)
			{
				...
				if(worldName != null)
				{
					throw new Exception($"Empty entity detected in world \"{worldName}\" after {_runSystems[i].GetType().Name}.Run().");
				}
				#endif
			}
		}
		...
		public virtual void Destroy()
		{
			for(var i = _allSystems.Count - 1; i >= 0; i--)
			{
				if(_allSystems[i] is IEcsDestroySystem destroySystem)
				{
					...
					if(worldName != null)
					{
						throw new Exception($"Empty entity detected in world \"{worldName}\" after {destroySystem.GetType().Name}.Destroy().");
					}
					#endif
				}
			}
			for(var i = _allSystems.Count - 1; i >= 0; i--)
			{
				if(_allSystems[i] is IEcsPostDestroySystem postDestroySystem)
				{
					...
					if(worldName != null)
					{
						throw new Exception($"Empty entity detected in world \"{worldName}\" after {postDestroySystem.GetType().Name}.PostDestroy().");
					}
					#endif
				}
			}
		...
		{
			if(systems.GetWorld().CheckForLeakedEntities())
			{
				return "default";
			}
