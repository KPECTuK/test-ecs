...
namespace Leopotam.EcsLite
{
	...
	public class EcsWorld
	{
		...
		private readonly List<IEcsWorldEventListener> _eventListeners;
		...
		{
			for(int ii = 0,
				iMax = _eventListeners.Count; ii < iMax; ii++)
			{
		...
		internal bool CheckForLeakedEntities()
		{
			...
			{
				for(int i = 0,
					iMax = _leakedEntities.Count; i < iMax; i++)
				{
		...
		public int NewEntity()
		{
			...
			else
			{
				...
				if(_entitiesCount * _entitiesItemSize == _entities.Length)
				{
					...
					for(int i = 0,
						iMax = _poolsCount; i < iMax; i++)
					{
						_pools[i].Resize(newSize);
					}
					for(int i = 0,
						iMax = _allFilters.Count; i < iMax; i++)
					{
		...
		public int GetAllEntities(ref int[] entities)
		{
			...
			var offset = 0;
			for(int i = 0,
				iMax = _entitiesCount; i < iMax; i++, offset += _entitiesItemSize)
			{
		...
		private (EcsFilter, bool) GetFilterInternal(Mask mask, int capacity = 512)
		{
			...
			for(int i = 0,
				iMax = mask.IncludeCount; i < iMax; i++)
			{
			...
			for(int i = 0,
				iMax = mask.ExcludeCount; i < iMax; i++)
			{
		...
		{
			for(int i = 0,
				iMax = filterMask.IncludeCount; i < iMax; i++)
			{
			...
			for(int i = 0,
				iMax = filterMask.ExcludeCount; i < iMax; i++)
			{
	...
	[Il2CppSetOption (Option.ArrayBoundsChecks, false)]
		...
		public sealed class Mask
		{
			...
			public EcsFilter End(int capacity = 512)
			{
				...
				for(int i = 0,
					iMax = IncludeCount; i < iMax; i++)
				{
					Hash = unchecked(Hash * 314159 + Include[i]);
				}
				for(int i = 0,
					iMax = ExcludeCount; i < iMax; i++)
				{
